...
import { 
  Plus, Settings, Trash2, X, Lock, CheckCircle2, ChevronUp, ChevronDown, RefreshCw, GripVertical
} from 'lucide-react';
import { genderService, GenderPayload } from '../utils/services/genderService';

export const GenderManagement: React.FC = () => {
  const [genders, setGenders] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [editingGender, setEditingGender] = useState<any | null>(null);
    ...
    try {
      for (let i = 0; i < tempList.length; i++) {
        ...
        if (Number(item.sort_order) !== newSortOrder) {
          await genderService.updateGender(item.id, {
            code: item.code,
            label: item.label,
  ...
  const fetchGenders = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.message || 'Failed to fetch genders.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    const payload: GenderPayload = {
      ...
      sort_order: Number(editingGender.sort_order) || 0
    };

    try {
      let res;
      if (editingGender.id) {
        res = await genderService.updateGender(editingGender.id, payload);
      } else {
        res = await genderService.createGender(payload);
      }

      if (res.success) {
        toast.success(res.message || 'Gender saved successfully.');
  ...
  const handleDelete = async (id: number | string, label: string) => {
    ...
    try {
      const res = await genderService.deleteGender(id);
      if (res.success) {
        toast.success(res.message || 'Gender deleted successfully.');
        fetchGenders();
      } else {
        toast.error(res.message || 'Failed to delete gender.');
      }
    } catch (err: any) {
      console.error(err);
      toast.error(err.message || 'Failed to delete gender.');
    }
  };

  const handleMove = async (index: number, direction: 'up' | 'down') => {
    const targetIndex = direction === 'up' ? index - 1 : index + 1;
    if (targetIndex < 0 || targetIndex >= genders.length) return;

    const newGenders = [...genders];
    
    // Swap sort orders
    const tempOrder = newGenders[index].sort_order;
    newGenders[index].sort_order = newGenders[targetIndex].sort_order;
    newGenders[targetIndex].sort_order = tempOrder;

    // Local swap for instant visual feedback
    const tempItem = newGenders[index];
    newGenders[index] = newGenders[targetIndex];
    newGenders[targetIndex] = tempItem;
    setGenders(newGenders);

    // Save changes to backend
    try {
      await genderService.updateGender(newGenders[index].id, {
        code: newGenders[index].code,
        label: newGenders[index].label,
        sort_order: Number(newGenders[index].sort_order)
      });
      await genderService.updateGender(newGenders[targetIndex].id, {
        code: newGenders[targetIndex].code,
        label: newGenders[targetIndex].label,
        sort_order: Number(newGenders[targetIndex].sort_order)
      });
      fetchGenders();
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-serif text-2xl text-[#0f281e]">Gender Options</h3>
          <p className="text-[#0f281e]/60 text-sm mt-1">Configure gender options used across user profiles and membership forms.</p>
        </div>
          ...
          <button
            onClick={() => {
              ...
              setEditingGender({
                code: '',
                label: '',
                sort_order: genders.length ? Math.max(...genders.map(g => Number(g.sort_order))) + 1 : 1
              });
            }}
                ...
                >
                  ...
                  <div className="col-span-2 text-right flex justify-end gap-2">
                    ...
                    <button
                      ...
                      className={`p-2 transition-colors ${g.is_system ? 'text-gray-200 cursor-not-allowed' : 'text-[#0f281e]/40 hover:text-red-500 cursor-pointer'}`}
                      title={g.is_system ? "System locked" : "Delete Option"}
                    >
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-serif text-2xl">
                      {editingGender.id ? 'Edit Gender' : 'Create Gender'}
                    </h3>
              ...
              <form onSubmit={handleCreateOrUpdate} className="space-y-4 p-6 sm:p-8">
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    required
                    value={editingGender.label}
                    onChange={e => setEditingGender({ ...editingGender, label: e.target.value })}
                ...
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    required
                    value={editingGender.code}
                    onChange={e => setEditingGender({ ...editingGender, code: e.target.value })}
                    placeholder="e.g. non_binary"
                    className="w-full rounded-xl border border-[#0f281e]/10 bg-[#0f281e]/[0.035] px-4 py-3 text-sm font-semibold text-[#0f281e] outline-none transition-colors focus:border-[#c4864b]"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    type="number"
                    required
                    value={editingGender.sort_order}
                    onChange={e => setEditingGender({ ...editingGender, sort_order: e.target.value })}
                    className="w-full rounded-xl border border-[#0f281e]/10 bg-[#0f281e]/[0.035] px-4 py-3 text-sm font-semibold text-[#0f281e] outline-none transition-colors focus:border-[#c4864b]"
                  />
