...
import { projectTypeService, ProjectTypePayload } from '../utils/services/projectTypeService';
...
export const ProjectTypeManagement: React.FC = () => {
    ...
    try {
      for (let i = 0; i < tempList.length; i++) {
        ...
        if (Number(item.sort_order) !== newSortOrder) {
          await projectTypeService.updateProjectType(item.id, {
            code: item.code,
            label: item.label,
  ...
  const fetchTypes = async () => {
    setIsLoading(true);
    try {
      const res = await projectTypeService.getProjectTypes();
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editingType.id) {
        res = await projectTypeService.updateProjectType(editingType.id, payload);
      } else {
        res = await projectTypeService.createProjectType(payload);
      }

      if (res.success) {
        toast.success(res.message || 'Project type saved successfully.');
        setEditingType(null);
        fetchTypes();
      } else {
        if (res.errors) {
        ...
        throw new Error(res.message || 'Failed to save project type.');
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleDelete = async (id: number | string, label: string) => {
    ...
    try {
      const res = await projectTypeService.deleteProjectType(id);
      if (res.success) {
        toast.success(res.message || 'Project type deleted successfully.');
        fetchTypes();
      } else {
        toast.error(res.message || 'Failed to delete project type.');
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    newTypes[index].sort_order = newTypes[targetIndex].sort_order;
    newTypes[targetIndex].sort_order = tempOrder;

    const tempItem = newTypes[index];
    newTypes[index] = newTypes[targetIndex];
    newTypes[targetIndex] = tempItem;
    setTypes(newTypes);

    try {
      await projectTypeService.updateProjectType(newTypes[index].id, {
        code: newTypes[index].code,
        label: newTypes[index].label,
        sort_order: Number(newTypes[index].sort_order)
      });
      await projectTypeService.updateProjectType(newTypes[targetIndex].id, {
        code: newTypes[targetIndex].code,
        label: newTypes[targetIndex].label,
        sort_order: Number(newTypes[targetIndex].sort_order)
      });
      fetchTypes();
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-serif text-2xl text-[#0f281e]">Project Type Options</h3>
          <p className="text-[#0f281e]/60 text-xs mt-1">Configure project type classifications for categorizing workflow pipelines.</p>
        </div>
          ...
          <button
            onClick={() => {
              ...
              setEditingType({
                code: '',
                label: '',
                sort_order: types.length ? Math.max(...types.map(t => Number(t.sort_order))) + 1 : 1
              });
            }}
                ...
                >
                  ...
                  <div className="col-span-2 text-right flex justify-end gap-2">
                    ...
                    <button
                      ...
                      className={`p-2 transition-colors ${t.is_system ? 'text-gray-200 cursor-not-allowed' : 'text-[#0f281e]/40 hover:text-red-500 cursor-pointer'}`}
                      title={t.is_system ? "System locked" : "Delete Option"}
                    >
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-serif text-2xl">
                      {editingType.id ? 'Edit Project Type' : 'Create Project Type'}
                    </h3>
              ...
              <form onSubmit={handleCreateOrUpdate} className="space-y-4 p-6 sm:p-8">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditingType({ ...editingType, sort_order: e.target.value })}
                    className="w-full rounded-xl border border-[#0f281e]/10 bg-[#0f281e]/[0.035] px-4 py-3 text-sm font-semibold text-[#0f281e] outline-none transition-colors focus:border-[#c4864b]"
                  />
