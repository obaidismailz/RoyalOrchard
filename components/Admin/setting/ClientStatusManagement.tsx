...
import { clientStatusService, ClientStatusPayload } from '../utils/services/clientStatusService';

export const ClientStatusManagement: React.FC = () => {
    ...
    try {
      for (let i = 0; i < tempList.length; i++) {
        ...
        if (Number(item.sort_order) !== newSortOrder) {
          await clientStatusService.updateClientStatus(item.id, {
            code: item.code,
            label: item.label,
  ...
  const fetchStatuses = async () => {
    setIsLoading(true);
    try {
      const res = await clientStatusService.getClientStatuses();
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editingStatus.id) {
        res = await clientStatusService.updateClientStatus(editingStatus.id, payload);
      } else {
        res = await clientStatusService.createClientStatus(payload);
      }

      if (res.success) {
        toast.success(res.message || 'Client status saved successfully.');
        ...
        throw new Error(res.message || 'Failed to save client status.');
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleDelete = async (id: number | string, label: string) => {
    ...
    try {
      const res = await clientStatusService.deleteClientStatus(id);
      if (res.success) {
        toast.success(res.message || 'Client status deleted successfully.');
        fetchStatuses();
      } else {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      await clientStatusService.updateClientStatus(newStatuses[index].id, {
        code: newStatuses[index].code,
        label: newStatuses[index].label,
        sort_order: Number(newStatuses[index].sort_order)
      });
      await clientStatusService.updateClientStatus(newStatuses[targetIndex].id, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-serif text-2xl text-[#0f281e]">Client Status Options</h3>
          <p className="text-[#0f281e]/60 text-xs mt-1">Configure status classifications managing customer accounts and CRM lead definitions.</p>
        </div>
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-serif text-2xl">
                      {editingStatus.id ? 'Edit Client Status' : 'Create Client Status'}
                    </h3>
