...
import { projectStatusService, ProjectStatusPayload } from '../utils/services/projectStatusService';

export const ProjectStatusManagement: React.FC = () => {
    ...
    try {
      for (let i = 0; i < tempList.length; i++) {
        ...
        if (Number(item.sort_order) !== newSortOrder) {
          await projectStatusService.updateProjectStatus(item.id, {
            code: item.code,
            label: item.label,
  ...
  const fetchStatuses = async () => {
    setIsLoading(true);
    try {
      const res = await projectStatusService.getProjectStatuses();
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editingStatus.id) {
        res = await projectStatusService.updateProjectStatus(editingStatus.id, payload);
      } else {
        res = await projectStatusService.createProjectStatus(payload);
      }

      if (res.success) {
        toast.success(res.message || 'Project status saved successfully.');
        ...
        throw new Error(res.message || 'Failed to save project status.');
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleDelete = async (id: number | string, label: string) => {
    ...
    try {
      const res = await projectStatusService.deleteProjectStatus(id);
      if (res.success) {
        toast.success(res.message || 'Project status deleted successfully.');
        fetchStatuses();
      } else {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      await projectStatusService.updateProjectStatus(newStatuses[index].id, {
        code: newStatuses[index].code,
        label: newStatuses[index].label,
        sort_order: Number(newStatuses[index].sort_order)
      });
      await projectStatusService.updateProjectStatus(newStatuses[targetIndex].id, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-serif text-2xl text-[#0f281e]">Project Status Options</h3>
          <p className="text-[#0f281e]/60 text-xs mt-1">Configure status categories managing overall project development lifecycles.</p>
        </div>
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-serif text-2xl">
                      {editingStatus.id ? 'Edit Project Status' : 'Create Project Status'}
                    </h3>
