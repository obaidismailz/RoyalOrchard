...
import { milestonePhaseService, MilestonePhasePayload } from '../utils/services/milestonePhaseService';

export const MilestonePhaseManagement: React.FC = () => {
    ...
    try {
      for (let i = 0; i < tempList.length; i++) {
        ...
        if (Number(item.sort_order) !== newSortOrder) {
          await milestonePhaseService.updateMilestonePhase(item.id, {
            code: item.code,
            label: item.label,
  ...
  const fetchPhases = async () => {
    setIsLoading(true);
    try {
      const res = await milestonePhaseService.getMilestonePhases();
      ...
      console.error(err);
      toast.error(err.message || 'Failed to fetch milestone phases.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    const payload: MilestonePhasePayload = {
      code: editingPhase.code || '',
      label: editingPhase.label || '',
      sort_order: Number(editingPhase.sort_order) || 0
    };

    try {
      let res;
      if (editingPhase.id) {
        res = await milestonePhaseService.updateMilestonePhase(editingPhase.id, payload);
      } else {
        res = await milestonePhaseService.createMilestonePhase(payload);
      }

      if (res.success) {
        toast.success(res.message || 'Milestone phase saved successfully.');
        ...
        throw new Error(res.message || 'Failed to save milestone phase.');
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleDelete = async (id: number | string, label: string) => {
    ...
    try {
      const res = await milestonePhaseService.deleteMilestonePhase(id);
      if (res.success) {
        toast.success(res.message || 'Milestone phase deleted successfully.');
        fetchPhases();
      } else {
        toast.error(res.message || 'Failed to delete milestone phase.');
      }
    } catch (err: any) {
      console.error(err);
      toast.error(err.message || 'Failed to delete milestone phase.');
    }
  };
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    const tempOrder = newPhases[index].sort_order;
    newPhases[index].sort_order = newPhases[targetIndex].sort_order;
    newPhases[targetIndex].sort_order = tempOrder;

    const tempItem = newPhases[index];
    newPhases[index] = newPhases[targetIndex];
    newPhases[targetIndex] = tempItem;
    setPhases(newPhases);

    try {
      await milestonePhaseService.updateMilestonePhase(newPhases[index].id, {
        code: newPhases[index].code,
        label: newPhases[index].label,
        sort_order: Number(newPhases[index].sort_order)
      });
      await milestonePhaseService.updateMilestonePhase(newPhases[targetIndex].id, {
        code: newPhases[targetIndex].code,
        label: newPhases[targetIndex].label,
        sort_order: Number(newPhases[targetIndex].sort_order)
      });
      fetchPhases();
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-serif text-2xl text-[#0f281e]">Milestone Phase Options</h3>
          <p className="text-[#0f281e]/60 text-xs mt-1">Configure project schedule lifecycle phases for milestones and tracking checkposts.</p>
        </div>
          ...
          <button
            onClick={() => {
              ...
              setEditingPhase({
                code: '',
                label: '',
                sort_order: phases.length ? Math.max(...phases.map(p => Number(p.sort_order))) + 1 : 1
              });
            }}
                ...
                >
                  ...
                  <div className="col-span-2 text-right flex justify-end gap-2">
                    ...
                    <button
                      ...
                      className={`p-2 transition-colors ${p.is_system ? 'text-gray-200 cursor-not-allowed' : 'text-[#0f281e]/40 hover:text-red-500 cursor-pointer'}`}
                      title={p.is_system ? "System locked" : "Delete Option"}
                    >
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-serif text-2xl">
                      {editingPhase.id ? 'Edit Milestone Phase' : 'Create Milestone Phase'}
                    </h3>
              ...
              <form onSubmit={handleCreateOrUpdate} className="space-y-4 p-6 sm:p-8">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditingPhase({ ...editingPhase, label: e.target.value })}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditingPhase({ ...editingPhase, sort_order: e.target.value })}
                    className="w-full rounded-xl border border-[#0f281e]/10 bg-[#0f281e]/[0.035] px-4 py-3 text-sm font-semibold text-[#0f281e] outline-none transition-colors focus:border-[#c4864b]"
                  />
