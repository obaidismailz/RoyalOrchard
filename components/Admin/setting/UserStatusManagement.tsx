...
import { 
  Plus, Settings, Trash2, X, Lock, CheckCircle2, ChevronUp, ChevronDown, GripVertical
} from 'lucide-react';
import { userStatusService, UserStatusPayload } from '../utils/services/userStatusService';

export const UserStatusManagement: React.FC = () => {
    ...
    try {
      for (let i = 0; i < tempList.length; i++) {
        ...
        if (Number(item.sort_order) !== newSortOrder) {
          await userStatusService.updateUserStatus(item.id, {
            code: item.code,
            label: item.label,
  ...
  const fetchStatuses = async () => {
    setIsLoading(true);
    try {
      const res = await userStatusService.getUserStatuses();
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editingStatus.id) {
        res = await userStatusService.updateUserStatus(editingStatus.id, payload);
      } else {
        res = await userStatusService.createUserStatus(payload);
      }

      if (res.success) {
        toast.success(res.message || 'User status saved successfully.');
        ...
        throw new Error(res.message || 'Failed to save status.');
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleDelete = async (id: number | string, label: string) => {
    ...
    try {
      const res = await userStatusService.deleteUserStatus(id);
      if (res.success) {
        toast.success(res.message || 'User status deleted successfully.');
        fetchStatuses();
      } else {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    // Local swap for visual response
    ...
    // Save changes
    try {
      await userStatusService.updateUserStatus(newStatuses[index].id, {
        code: newStatuses[index].code,
        label: newStatuses[index].label,
        sort_order: Number(newStatuses[index].sort_order)
      });
      await userStatusService.updateUserStatus(newStatuses[targetIndex].id, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-serif text-2xl text-[#0f281e]">User Status Options</h3>
          <p className="text-[#0f281e]/60 text-xs mt-1">Configure status scopes that manage access and invite lifecycles for admin profiles.</p>
        </div>
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-serif text-2xl">
                      {editingStatus.id ? 'Edit User Status' : 'Create User Status'}
                    </h3>
